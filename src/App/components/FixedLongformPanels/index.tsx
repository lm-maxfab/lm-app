...
class FixedLongformPanels extends Component<Props, State> {
  ...
  isReadyToPlayAnimation: boolean = false
  state: State = {
    hasPlayedLogoAnimation: false
  }

  constructor (props: Props) {
    super(props)
    window.setTimeout(() => {
      this.isReadyToPlayAnimation = true
    }, 500)
  }
  ...
  render (): JSX.Element {
    ...
    const introParagraphs = props.introImages?.map((introImage, introImagePos) => {
      ...
      const paragraphSpans = paragraphContent
        ?.split('[[SPLIT]]')
        .filter(e => e !== '')
        .map(e => <StrToHtml content={e} wrapperTag='span' />)

      let spanClass = 'frag-intro-paragraph-chunk'
      if (activePanel === null) spanClass += ''
      else if (introImagePos === activePanel) spanClass += ' frag-intro-paragraph-chunk_current'
      else if (introImagePos < activePanel) spanClass += ' frag-intro-paragraph-chunk_passed'
      else if (introImagePos > activePanel) spanClass += ' frag-intro-paragraph-chunk_to-come'
      return <span className={spanClass}>{paragraphSpans}</span>
    })

    let introClasses = `${this.mainClass}__panel ${this.mainClass}__intro-panel`
...
export type { Props }
export default FixedLongformPanels
